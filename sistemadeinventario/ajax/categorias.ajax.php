...
class AjaxCategorias{
	...
	public $idcategoria; //lo hacemos public,porque aqui vamos a recibir la variable idUsuario desde el archivo usuarios.js
	...
	public function AjaxEditarCategorias(){
		...
		$valor= $this->idCategoria; //NOTA:es importante que cuando usemos el ->,la palabra,variable o lo que sea que siga,este pegado a este ->,es decir,no deben haber espacios ni antes del ->,ni despues de el.
...
if(isset($_POST["idCategoria"])) { //DECIMOS QUE si el post,envia idUsuarios,entonces que ejecute el siguiente codigo:


	$editar= new AjaxCategorias(); //instanciamos la clase AjaxUsuarios,que es esta clase,la cual contiene la funcion que vamos a llamar.
	$editar->idCategoria= $_POST['idCategoria'];//aqui decimos que idUsuario se vaya a el objeto instanciado,que es la funcion AjaxUsuarios
	$editar->AjaxEditarCategorias(); //luego decimos que la variable editar que contiene el id usuario,llame a la funcion AjaxEditarUsuarios,haciendo que se ejecute dicha funcion.

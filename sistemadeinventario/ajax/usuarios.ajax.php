...
if(isset($_POST["idUsuario"])) { //DECIMOS QUE si el post,envia idUsuarios,entonces que ejecute el siguiente codigo:
	...
	$editar= new AjaxUsuarios(); //instanciamos la clase AjaxUsuarios,que es esta clase,la cual contiene la funcion que vamos a llamar.
	$editar->idUsuario= $_POST['idUsuario'];//aqui decimos que idUsuario se vaya a el objeto instanciado,que es la funcion AjaxUsuarios
	$editar->AjaxEditarUsuarios(); //luego decimos que la variable editar que contiene el id usuario,llame a la funcion AjaxEditarUsuarios,haciendo que se ejecute dicha funcion.

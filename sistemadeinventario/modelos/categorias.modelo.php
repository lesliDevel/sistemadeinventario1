...
class ModeloCategorias{
	...
	static public function mdlGuardarCategorias($tabla,$datos){
		...
		$validacion= Conexion::conectar()->prepare("SELECT * FROM $tabla WHERE nombre = :nombre");
		$validacion->bindParam(":nombre",$datos["nombre"],PDO::PARAM_STR);
		...
		$stmt->null; //aqui lo que hacemos es dejar vacia la variabke stmt haciendola igual a null,por motivos de seguridad
			...
			echo "Error inesperado,verifique que la categoria ingresada sea diferente a las que estan registrados en la base de datos:".$e;
				return "error";
...
//aqui llegan desde usuarios.controlador,la llamada y los parametros,los cuales se van a almacenar en los parametros de aqui,en tabla se va a almacenar el nombre de la tabla,en item la variable item,la cual es null,en valor,la variable valor,la cual es null.
	static public function mdlMostrarCategorias($tabla,$item,$valor){ //declaramos esta funcion y declaramos dentro de las 
	...
	static public function mdlEditarCategorias($tabla,$datos){ //recibimos las variables tabla y datos
	...
	static public function mdlBorrarCategorias($tabla,$datos){ //la funcion mdlBorrarUsuarios,es llamada por la funcion crtBorrarUsuarios,la cual le envia a esta funcion los parametros tabla,que contiene el nombre de la tabla y datos que contiene el id del usuario que vamos a eliminar.
